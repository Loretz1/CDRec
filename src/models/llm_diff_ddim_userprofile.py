...
class LLM_Diff_ddim_userprofile(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(LLM_Diff_ddim_userprofile, self).__init__(config, dataloader)
        ...
        semantic_emb = dataloader.get_modality_embs()['CrossDomain_semantics']
...
def extract_CrossDomain_semantics_modality_data(config, modality, interaction, id_mapping, raw_data_list):
...
def generate_CrossDomain_semantics_embs(
    config,
    modality,
    interaction,
    id_mapping,
...
def generate_CrossDomain_semantics_final_embs(config, modality, interaction, id_mapping, modality_embs):
...
You are an expert in cross-domain user modeling, specializing in summarizing user interests. The goal is to generate one concise summary of the user's interests that is useful for recommendation in BOTH domains, based on the user's interactions, item information, and reviews from the two domains. The source domain focuses on items related to clothing, footwear, and personal accessories, while the target domain focuses on items related to sports, fitness, and outdoor activities.

The information I will give you:
SOURCE DOMAIN INTERACTIONS: A LIST of user interactions in the source domain.
TARGET DOMAIN INTERACTIONS: A LIST of user interactions in the target domain.

Each interaction is described in JSON format with the following attributes, where missing values are set to "None". The attributes include the item's information and the user's review on that item:
{
...
Requirements:
1. Please think step by step to analyze the user's interests based on the interactions in BOTH domains:
   - Identify recurring themes, topics, styles, and preference signals implied by item attributes and reviews.
   - Derive the user's general preferences and tendencies that are reflected across interactions in both domains.
   - Abstract them into a high-level, cross-domain interests.
   - Remove domain-specific entities, formats, or terminology and do not output the thinking process.
2. Please provide your answer in JSON format, following this structure:
{
  "summary": "A concise summary of the user's interests that is useful for recommendation in both domains"
}
3. The summary must be no more than 130 tokens.
