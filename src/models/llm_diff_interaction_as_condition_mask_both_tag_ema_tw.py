...
class LLM_Diff_interaction_as_condition_mask_both_tag_EMA_Tw(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(LLM_Diff_interaction_as_condition_mask_both_tag_EMA_Tw, self).__init__(config, dataloader)
    ...
    def calculate_loss(self, interaction, epoch_idx):
        ...
        if use_pseudo:
            lambda_t = min(1, epoch_idx / self.config["T_w"])
            loss = loss + lambda_t * pseudo_loss
        return loss
