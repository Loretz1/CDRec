...
class LLM_Diff_ddim_one_diff(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(LLM_Diff_ddim_one_diff, self).__init__(config, dataloader)
    ...
    def calculate_loss(self, interaction, epoch_idx):
        ...
        diff_loss_src, u_src_denoised = self.diff.p_losses(
            x_start=u_src,
            t=t_src,
            loss_type="l2"
        )
        ...
        diff_loss_tgt, u_tgt_denoised = self.diff.p_losses(
            x_start=u_tgt,
            t=t_tgt,
            loss_type="l2"
        )
