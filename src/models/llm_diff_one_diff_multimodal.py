...
class LLM_Diff_one_diff_multimodal(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(LLM_Diff_one_diff_multimodal, self).__init__(config, dataloader)
    ...
    def calculate_loss(self, interaction, epoch_idx):
        ...
        # 这里t是随机采的，不是对称采样
        B = u_src.size(0)
        t_src = torch.randint(low=0, high=self.diff.timesteps, size=(B,), device=u_src.device)
        # diff_loss_src, u_src_denoised = self.diff_src.p_losses(x_start=u_src, t=t_src, loss_type="l2")
        diff_loss_src, u_src_denoised = self.diff.p_losses(x_start=u_src, t=t_src, loss_type="l2")
        ...
        B = u_tgt.size(0)
        t_tgt = torch.randint(low=0, high=self.diff.timesteps, size=(B,), device=u_tgt.device)
        # diff_loss_tgt, u_tgt_denoised = self.diff_tgt.p_losses(x_start=u_tgt, t=t_tgt, loss_type="l2")
        diff_loss_tgt, u_tgt_denoised = self.diff.p_losses(x_start=u_tgt, t=t_tgt, loss_type="l2")
    ...
    def full_sort_predict(self, interaction, is_warm):
        ...
        if is_warm:
            ...
            _, u_denoised, _, _, _ = self.diff.sample(u)
        else:
...
def extract_LLM_Diff_one_diff_multimodal_semantics_modality_data(config, modality, interaction, id_mapping, raw_data_list):
    ...
    # 首先分别整理src用户，tgt用户在各自域的评论+物品信息
    src_user_item_profiles = {}
    tgt_user_item_profiles = {}
    ...
    for row in interaction['src'].itertuples(index=False):
        ...
        if raw_user_id not in src_user_item_profiles:
            src_user_item_profiles[raw_user_id] = []
        ...
        # merge meta + review
        item_profile_with_review = {
            **item_profile,   # 物品meta信息
            "review": review, # 用户评论
            "raw_item_id": raw_item_id,  # 物品raw id，用于debug
        }
        src_user_item_profiles[raw_user_id].append(item_profile_with_review)

    # src用户数量一致
    assert len(src_user_item_profiles) == len(id_mapping['src']['user2id'])
    ...
    for row in interaction['tgt'].itertuples(index=False):
        ...
        if raw_user_id not in tgt_user_item_profiles:
            tgt_user_item_profiles[raw_user_id] = []
        ...
        tgt_user_item_profiles[raw_user_id].append(item_profile_with_review)

    # tgt用户数量一致
    assert len(tgt_user_item_profiles) == len(id_mapping['tgt']['user2id'])
