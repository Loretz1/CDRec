...
class UniCDR_only_shared_emb(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(UniCDR_only_shared_emb, self).__init__(config, dataloader)
    ...
    def calculate_loss(self, interaction, epoch_idx):
        ...
        user_src_emb = self.agg(user_emb_src, global_item_emb_src, global_score_src)
        user_src_emb = F.dropout(user_src_emb, self.dropout, training=self.training)
        ...
        user_tgt_emb = self.agg(user_emb_tgt, global_item_emb_tgt, global_score_tgt)
        user_tgt_emb = F.dropout(user_tgt_emb, self.dropout, training=self.training)
        ...
        loss = loss_bpr_src + loss_bpr_tgt
        return loss
    ...
    def full_sort_predict(self, interaction, is_warm):
        ...
        user_repr = self.agg(user_emb, global_item_emb, global_score)

        scores = torch.matmul(user_repr, self.emb_item_tgt.weight.t())
        scores[:, 0] = 0.0
        return scores
